...
for useSIM in [2]:
    ...
    stn_gpe_factor_idx = 3
    experiment = ["001e", "001f", "001g", "014a", "014b", "014c"][experiment_idx]
    nr_sims = {"001e": 60, "001f": 60, "001g": 60, "014a": 60, "014b": 60, "014c": 60}[
        experiment
    ]
    dataFolder = {}
    dataFolder[
        "shortSimulation"
    ] = "../../../simulations/007a_plotSimulation_learningrule_PaperLearningRule/2_dataRaw"
    dataFolder["wholeSimulation"] = {
    ...
    trialIdx = (
        {}
    )  # over which trials the mw changes should be calculated, took from output file
    ...
    trialIdx["wholeSimulation_after"] = np.zeros(nr_sims).astype(int)
    for idx_sim in range(nr_sims):
        trialIdx["wholeSimulation_after"][idx_sim] = get_lasttrial(
            dataFolder["wholeSimulation"], idx_sim + 1
        )
    activityTime = (
        {}
    )  # monitor start end, after selection start end, to compare activity after selection with global activitie
    activityTime["start_global"] = 13111.8
    activityTime["end_global"] = 127502.5
    activityTime["delta_global"] = (
        activityTime["end_global"] - activityTime["start_global"]
    )
    activityTime["start_afterRS"] = 125802.5  # about when dopamine starts
    activityTime["delta_afterRS"] = 300
    activityTime["end_afterRS"] = (
        activityTime["start_afterRS"] + activityTime["delta_afterRS"]
    )  # about when dopamine ends
    subPopsSize = 100
    ### Figure
    fig = {}
    fig["B"] = 180  # mm
    ...
    circles["gab"] = 3.6
    circles["randomDots"] = np.array(
        [
            np.array([0.25, 0.25, 0.25, 0.6, 0.6, 0.6, 0.83, 0.83, 0.83, 0.83])
            * circles["radius"],
            np.array([0, 0.4, 0.8, 0.2, 0.7, 0.9, 0.1, 0.4, 0.6, 0.8]) * 2 * np.pi,
        ]
    )
    rectangle = {}
    ...
    cmap_activities = LinearSegmentedColormap.from_list(
        "mycmap", ["blue", "white", "orange"]
    )
    param_mw_max = 0.31
    ...
    mw["wholeSimulation"] = {}
    for idx_sim in range(nr_sims):
        mw["wholeSimulation"][str(idx_sim)] = np.load(
            f'{dataFolder["wholeSimulation"]}/mw_stn_gpe_sim{idx_sim+1}.npy'
        )
    mw["shortSimulation"] = np.load(
        dataFolder["shortSimulation"]
        + "/mw_stn_gpe_sim"
        + str(SimIdx["shortSimulation"])
        + ".npy"
    )
    mw["wholeSimulationTransformation"] = {}
    for idx_sim in range(nr_sims):
        mw["wholeSimulationTransformation"][str(idx_sim)] = (
            mw["wholeSimulation"][str(idx_sim)][0] / initialWeights
        )
    mw["shortSimulationTransformation"] = mw["shortSimulation"][0] / initialWeights
    spikeTimes = {}
    spikeTimes["GPe"] = np.load(
        dataFolder["shortSimulation"]
        + "/t_gpe_sim"
        + str(SimIdx["shortSimulation"])
        + ".npy"
    )
    spikeTimes["STN"] = np.load(
        dataFolder["shortSimulation"]
        + "/t_stn_sim"
        + str(SimIdx["shortSimulation"])
        + ".npy"
    ...
    spikeCounts = {}
    spikeCounts["GPe"] = np.load(
        dataFolder["shortSimulation"]
        + "/n_gpe_sim"
        + str(SimIdx["shortSimulation"])
        + ".npy"
    )
    spikeCounts["STN"] = np.load(
        dataFolder["shortSimulation"]
        + "/n_stn_sim"
        + str(SimIdx["shortSimulation"])
        + ".npy"
    ...
    for rewardedAction in ["2", "3", "4"]:
        mwChanges[rewardedAction] = (
            mw["shortSimulation"][trialIdx[rewardedAction + "_after"]]
            / mw["shortSimulationTransformation"]
            - mw["shortSimulation"][trialIdx[rewardedAction + "_before"]]
            / mw["shortSimulationTransformation"]
        ).transpose() / initialWeights  # initialWeights  # dwMax

    ### Meanweight changes of whole simulation
    mwChanges["wholeSimulation"] = {}
    for idx_sim in range(nr_sims):
        mwChanges["wholeSimulation"][str(idx_sim)] = (
            mw["wholeSimulation"][str(idx_sim)][
                trialIdx["wholeSimulation_after"][idx_sim]
            ]
            / mw["wholeSimulationTransformation"][str(idx_sim)]
            - mw["wholeSimulation"][str(idx_sim)][trialIdx["wholeSimulation_before"]]
            / mw["wholeSimulationTransformation"][str(idx_sim)]
        ).transpose() / initialWeights  # initialWeights  # dwMax

    ### average mwChanges["wholeSimulation"]
    mwChanges_list = []
    for idx_sim in range(nr_sims):
        mwChanges_list.append(mwChanges["wholeSimulation"][str(idx_sim)])
    mwChanges["wholeSimulation"] = np.mean(np.array(mwChanges_list), 0)

    ### Min max changes for color symmetrical code
    if isinstance(param_mw_max, type(None)):
        mw_max = np.amax(
            [np.amax(np.abs(changes)) for changes in list(mwChanges.values())]
        )
    else:
        mw_max = param_mw_max
    mw_min = -mw_max
    ...
    trials, correct, decision, frequentAction, start, dopInp = get_output(
        dataFolder["shortSimulation"], SimIdx["shortSimulation"]
    )
    ...
    after_selection_times = dopInp[
        (decision == selected_action) * (correct == selected_action)
    ][
        starting_trial:
    ]  # all times of dop inputs where action 2 was correctly selected
    ...
    for popIdx, popName in enumerate(["GPe", "STN"]):
        allTimes = np.array(
            [spikeTimes[popName] for i in range(after_selection_times.shape[0])]
        )  # duplicate spike times array, to compare it with after_selection_times
        spike_time_windows = np.sum(
            (allTimes > after_selection_times[:, None])
            * (allTimes < after_selection_times[:, None] + timeAfterSelection),
            0,
        )  # get for each trial the spiketimes after selection
        spikeCountsMasked = spikeCounts[popName][
            np.array(spike_time_windows, dtype=bool)
        ]  # all spikes which occured in all this time (after selection periods of all trials)
        for subPop in range(5):  # deviding spikes in 5 sub populations
            activities["afterRS"][subPop, popIdx] = (
                np.sum(
                    (spikeCountsMasked >= 100 * subPop)
                    * (spikeCountsMasked < 100 * (subPop + 1))
                )
                / (timeAfterSelection * after_selection_times.shape[0] / 1000.0)
                / subPopsSize
            )
    ### then obtain activity over complete simulation time
    global_time = np.array(
        [
            start[(decision == selected_action) * (correct == selected_action)][
                starting_trial
            ],
            after_selection_times[-1],
        ]
    )  # the complete time range beginning to end of aciton 2 was correctly selected
    complete_simulation_time = global_time[1] - global_time[0]
    activities["global"] = np.zeros((5, 2))
    for popIdx, popName in enumerate(["GPe", "STN"]):
        mask = (spikeTimes[popName] > global_time[0]) * (
            spikeTimes[popName] < global_time[1]
        )
        spikeCountsMasked = spikeCounts[popName][np.array(mask, dtype=bool)]
        for subPop in range(5):  # deviding spikes in 5 sub populations
            activities["global"][subPop, popIdx] = (
                np.sum(
                    (spikeCountsMasked >= 100 * subPop)
                    * (spikeCountsMasked < 100 * (subPop + 1))
                )
                / (complete_simulation_time / 1000.0)
                / subPopsSize
            )
    ### compare both activity vectors
    activities["afterRS"] = (activities["afterRS"] - activities["global"]) / activities[
        "global"
    ]
    ...
    ax_A.axis("off")
    plt.text(
        ax_A.get_position().x0 - 0.1,
        (ax_A.get_position().y0 + ax_A.get_position().y1) / 2.0,
        "A:",
    ...
    grid = (
        np.concatenate(
            (
                gridX,
                gridX,
                np.ones(gridX.shape[0]) * circles["gridyMax"],
                np.ones(gridX.shape[0]) * circles["gridyMin"],
            )
        )
        .reshape((2, 2 * gridX.shape[0]))
        .T
    )
    ...
    for idx, point in enumerate(grid):  # left
        plt.plot(
            point[0]
            - circles["gab"] / 2.0
            + circles["radius"] * np.cos(circles["angles"]),
            point[1] + circles["radius"] * np.sin(circles["angles"]),
            color=colorList[idx],
        )
        plt.plot(
            point[0]
            - circles["gab"] / 2.0
            + circles["randomDots"][0] * np.cos(circles["randomDots"][1]),
            point[1] + circles["randomDots"][0] * np.sin(circles["randomDots"][1]),
            ".",
            color=colorList[idx],
        )
    ...
    ### draw 4 rectangles
    for rec in range(4):
        offsetX = [circles["gab"] / 2.0, -circles["gab"] / 2.0][rec // 2]
        offsetY = [0, circles["gridyMax"] - circles["gridyMin"]][rec % 2]
        plt.plot(
            [
                rectangle["x0"] + offsetX,
    ...
    ### write STN, GPe
    plt.text(
        0, circles["gridyMax"], "GPe", va="center", ha="center", **font["STNGPeLabels"]
    )
    plt.text(
        0, circles["gridyMin"], "STN", va="center", ha="center", **font["STNGPeLabels"]
    )

    ### draw arrows
    def sizedArrow(x0, y0, x1, y1, width, color, style):
        if style == "LTP":
            sizedArrowStyle = mtl.patches.ArrowStyle.Simple(
                tail_width=1 * width, head_width=3 * width, head_length=5 * width
            )
            arrow = mtl.patches.FancyArrowPatch(
                (x0, y0),
                (x1, y1),
                ...
                clip_on=False,
                **dict(
                    arrowstyle=sizedArrowStyle, linewidth=0.5 + 0.5 * width, color=color
                ),
            )
            plt.gca().add_patch(arrow)
        elif style == "LTD":
            xHead = x1
            yHead = y1 - (width / 30.0) * 1.4
            plt.gca().add_patch(
                mtl.patches.Circle(
                    (xHead, yHead), width / 30.0, fill=True, color=color, clip_on=False
                )
            )
            sizedArrowStyle = mtl.patches.ArrowStyle.Simple(
                tail_width=1 * width, head_width=1 * width, head_length=0.01 * width
            )
            arrow = mtl.patches.FancyArrowPatch(
                (x0, y0),
                (xHead, yHead),
    ...
    ### legend
    xLeft = -legend["width"] / 2.0
    legendField = mtl.patches.FancyBboxPatch(
        xy=(xLeft, legend["bottom"]),
        width=legend["width"],
        height=legend["height"],
        boxstyle=mtl.patches.BoxStyle.Round(pad=0.1),
    ...
    # LTP arrow
    (x0, y0, x1, y1, textdist) = (
        -legend["width"] / 2.0 + legend["freespace"],
        legend["bottom"] + legend["height"] / 2.0,
        -legend["width"] / 2.0 + legend["freespace"] + legend["arrowlength"],
        legend["bottom"] + legend["height"] / 2.0,
        legend["freespace"],
    )
    sizedArrow(x0, y0, x1, y1, 1, "k", "LTP")
    ...
    (x0, y0, x1, y1, textdist) = (
        legend["freespace"],
        legend["bottom"] + legend["height"] / 2.0,
        legend["freespace"] + legend["arrowlength"],
        legend["bottom"] + legend["height"] / 2.0,
        legend["freespace"],
    )
    ...
    plt.xlim(
        rectangle["x0"] - circles["gab"] / 2.0 - activitiesPlotBorder,
        rectangle["x1"] + circles["gab"] / 2.0 + activitiesPlotBorder,
    )
    plt.ylim(
        rectangle["y0"], rectangle["y1"] + circles["gridyMax"] - circles["gridyMin"]
    )
    ...
    for idx, weightchange in enumerate(["2", "3", "4", "wholeSimulation"]):
        ...
        image = ax.matshow(
            mwChanges[weightchange], cmap=cmap_dw, vmin=mw_min, vmax=mw_max
        )
        if weightchange == "wholeSimulation":
            with open("../3_results/values.txt", "a") as f:
                print(experiment, file=f)
                print(mwChanges[weightchange], file=f)
                print("\n", file=f)
        ...
        plt.text(
            ...
            (ax.get_position().y0 + ax.get_position().y1) / 2.0,
            subplotLabels[idx],
            transform=plt.gcf().transFigure,
            **font["subplotLabels"],
        )
    ...
    plt.yticks(
        cax_weights.get_yticks(), np.round(np.array(cax_weights.get_yticks()) * 100, 0)
    )
    # cbar_weights.update_ticks()
    # print(cax_weights.get_yticks())

    ### colorbar activities
    cax_activities = plt.axes(
        [
            0.85,
            (ax_A.get_position().y1 - ax_A.get_position().y0 - colorbarH_activities)
            / 2.0
            + ax_A.get_position().y0,
            0.03,
            colorbarH_activities,
        ]
    )
    plt.colorbar(
        mtl.cm.ScalarMappable(norm=mtl.colors.Normalize(vmin=-1, vmax=1), cmap=cmap),
        cax=cax_activities,
    )
    ...
    plt.savefig(f"../3_results/activities_and_weightchanges_{experiment}.svg")
