...
def plot_column(
    ...
    if not (weight_plot):
        ...
        if mode == "scatter":
            if col == 0:
                x_scatter, y_scatter, s_scatter, c_scatter = selections_to_scatter(
                    selections, trials, size=10, return_counts=True, max_nr=60
                )
                selections_raw = selections[
                    np.logical_not(np.isnan(selections[:, 0])), :
                ]
                ...
                (
                    ...
                    selections[0], trials, size=10, return_counts=True, max_nr=60
                )
                (
                    x_scatter_new,
                    y_scatter_new,
                    ...
                    selections[1], trials, size=10, return_counts=True, max_nr=60
                )
                ...
                data_set_pre = [
                    selections[0][:, violin_idx]
                    for violin_idx, trial in enumerate(trials)
                ]
                data_set_new = [
                    selections[1][:, violin_idx]
                    for violin_idx, trial in enumerate(trials)
                ]
            ...
            if col == 0:
                ...
                shading = False
                if shading:
                    ### mark trials by shading
                    for xpos_idx, xpos in enumerate(trials):
                        if np.mod(xpos_idx, 2) == 0:
                            plt.axvspan(
                                xmin=np.clip(xpos - 0.5, x_min_lim, x_max_lim),
                                xmax=np.clip(xpos + 0.5, x_min_lim, x_max_lim),
                                facecolor="w",
                                edgecolor=None,
                                alpha=0.1,
                            )
        ...
        plt.xlabel("trials", **bold_font)
        if col == 0:
            plt.ylabel("Hyperdirect", **bold_font)
        if col == 1:
            ax.set_yticklabels([])
            # plt.legend()
