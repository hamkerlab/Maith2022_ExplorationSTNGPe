...
experiment = ["001e", "001f", "001g", "014a", "014b", "014c", "002e"][experiment_idx]
num_sims = {
    "001e": 60,
    "001f": 60,
    "001g": 60,
...
folder = {
    ...
    "002e": "../../../simulations/002e_Cluster_Experiment_PaperLearningRule_LearningOFF/4_dataEv",
}[experiment]
...
for sim_id in range(num_sims):
    ...
    number_errors[sim_id] = (
        np.sum(np.logical_not(block_is_new_correct), 1) - 1
    )  # sum of trials which are not new correct (-1 because of pre switch trial)

    ### WEIGHTS
    switch_weights_sd1[sim_id] = get_switch_weights(
        mw_sd1[1 : trials + 1].reshape(trials, 5), post_switch_trials, correct
    )
    switch_weights_sd2[sim_id] = get_switch_weights(
        mw_sd2[1 : trials + 1].reshape(trials, 5), post_switch_trials, correct
    )
    switch_weights_stn[sim_id] = get_switch_weights(
        mw_stn[1 : trials + 1].reshape(trials, 5), post_switch_trials, correct
    )
    ...
    ### WEIGHTS
    initial_weights_sd1[sim_id] = get_initial_weights_initial_learning(
        mw_sd1[:trials].reshape(trials, 5), post_switch_trials, correct
    )
    initial_weights_sd2[sim_id] = get_initial_weights_initial_learning(
        mw_sd2[:trials].reshape(trials, 5), post_switch_trials, correct
    )
    initial_weights_stn[sim_id] = get_initial_weights_initial_learning(
        mw_stn[:trials].reshape(trials, 5), post_switch_trials, correct
    )
...
with open(f"../3_results/quantities_{experiment}.txt", "w") as f:
    ...
    print(
        "initial:",
        np.mean(number_initial_errors),
        np.std(number_initial_errors),
        file=f,
    )
    print(
        "reversal:",
        np.mean(number_errors_per_sim),
        np.std(number_errors_per_sim),
        "\n",
        file=f,
    )
    ...
    print(
        "reversal:",
        np.mean(pre_correct_number, 0),
        np.mean(new_correct_number, 0),
        file=f,
    )
...
### SAVES FOR STATS
save_2_within_arrays(
    f"number_of_errors_sims_{experiment}.txt",
    "ERRORS",
    number_initial_errors,
    number_errors_per_sim,
)
