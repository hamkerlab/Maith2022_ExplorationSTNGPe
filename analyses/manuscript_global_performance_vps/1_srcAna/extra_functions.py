...
def get_response_times(start, dopInp, decision, correct, block, experiment):
    """
    ...
    block_start_end_list = remove_invalid_blocks(block_start_end_list, experiment)

    ### from these blocks: which are successfull
    successful_blocks = get_successful_block_list(
        correct, decision, block, experiment
    ).astype(bool)
    block_start_end_list = block_start_end_list[successful_blocks]

    ### get response times only of the valid blocks
    response_times_valid = []
    for start, end in block_start_end_list:
        response_times_valid.append(response_times[start : end + 1])

    ret = np.concatenate(response_times_valid)
...
def get_block_trial_indizes(correct, block, post_switch_trials, experiment):
    ...
    ### -1 because first returned trial shoul be the last of the prev rewarded ones
    switch_trials = get_switch_trials(correct, block) - 1

    ### exclude rule switches which are shortly before breaks or between the two blocks
    switch_trials = remove_invalid_ruleswitches(switch_trials, experiment)
...
def selections_to_scatter(
    selections, trials, size=10, return_counts=False, max_nr=None
):
    """
    ...
    assert (
        trials.size == selections.shape[1]
    ), "ERROR: selections_to_scatter, trials size does not fit to selections array"
...
def plot_column(
    ...
    """
    trials = np.arange(-1, post_switch_trials).astype(int)

    wrong_color = (216 / 255, 27 / 255, 96 / 255)
    correct_color = (30 / 255, 136 / 255, 229 / 255)

    if mode == "scatter":
        if col == 0:
            x_scatter, y_scatter, s_scatter, c_scatter = selections_to_scatter(
                selections, trials, size=10, return_counts=True, max_nr=10
            )
            selections_raw = selections[np.logical_not(np.isnan(selections[:, 0])), :]
            selections_sd = np.nanstd(selections, 0)
            ...
            (
                ...
                s_scatter_pre,
                c_scatter_pre,
            ) = selections_to_scatter(
                selections[0], trials, size=10, return_counts=True, max_nr=10
            )
            (
                x_scatter_new,
                y_scatter_new,
                ...
                selections[1], trials, size=10, return_counts=True, max_nr=10
            )
            selections_pre = np.mean(selections[0], 0)
            selections_new = np.mean(selections[1], 0)
            selections_pre_sd = np.std(selections[0], 0)
            selections_new_sd = np.std(selections[1], 0)

            ### get violin plot data
            data_set_pre = [
                selections[0][:, violin_idx] for violin_idx, trial in enumerate(trials)
            ]
            data_set_new = [
                selections[1][:, violin_idx] for violin_idx, trial in enumerate(trials)
            ]

        ### FIRST ROW
        ax = plt.subplot(3, 2, col + 1)
        plt.title(title, **large_bold_font)
        x_min_lim, x_max_lim = [-1.3, post_switch_trials - 0.7]
        if col == 0:
            ...
            axo.errorbar(
                x=trials,
                y=selections,
                yerr=selections_sd * 0,  # sd to large?
                fmt="k-",
                alpha=1,
            )
            ...
            # selections_com = np.append(selections_raw, selections).reshape(
            #     selections_raw.shape[0] + 1, selections_raw.shape[1]
            # )
            ...
            cmap = create_cm([wrong_color, correct_color])
            plt.imshow(
                selections_raw,
                cmap=cmap,
                aspect="auto",
            ...
            if grid:
                ### mark trials by grid
                for xpos in trials:
                    plt.axvline(xpos + 0.5, color="k", lw=0.3)
            ...
            if not (violin):
                ...
                plt.scatter(
                    ...
                    y_scatter_pre,
                    s=s_scatter_pre,
                    facecolor=wrong_color + (0.5,),
                    edgecolor=wrong_color + (1,),
                    lw=0.2 * np.sqrt(s_scatter_pre),
                )
                plt.scatter(
                    x_scatter_new,
                    y_scatter_new,
                    s=s_scatter_new,
                    facecolor=correct_color + (0.5,),
                    edgecolor=correct_color + (1,),
                    lw=0.2 * np.sqrt(s_scatter_new),
                )

                ### add annotation "pre/worng"
                ...
                ### make violin plots
                parts_pre = plt.violinplot(
                    dataset=data_set_pre,
                    positions=trials,
                    showextrema=False,
                    showmeans=False,
                    showmedians=False,
                )
                parts_new = plt.violinplot(
                ...
                for pc in parts_pre["bodies"]:
                    pc.set_facecolor(wrong_color)
                    pc.set_edgecolor(None)
                    pc.set_alpha(1)
                for pc in parts_new["bodies"]:
                    pc.set_facecolor(correct_color)
                    pc.set_edgecolor(None)
                    pc.set_alpha(1)
        ...
        if col == 0:
            axo.set_ylabel("Performance", **bold_font)
        if col == 1:
            ax.set_yticklabels([])
        ...
        if col == 0:
            ...
            selections_pre_er = stats.sem(
                selections[0], axis=0, ddof=0, nan_policy="omit"
            )
            selections_new_er = stats.sem(
                selections[1], axis=0, ddof=0, nan_policy="omit"
            )
            selections_pre = np.mean(selections[0], 0)
            selections_new = np.mean(selections[1], 0)
...
def get_exploration_idx(correct, decision, block, experiment):
    """
...
def get_reversal_learning_idx(correct, decision, block, experiment):
    """
...
def get_consecutive_rewarded_idx(correct, decision, block, experiment):
    """
    ...
    for idx, correct_decisions in enumerate(correct_decision_block):
        consecutive_rewarded_block.append(
            get_consecutive_rewarded_block(
                correct_decisions, decision_block[idx], prev_correct[idx]
            )
        )
...
def get_exploration_block(block_correct_decisions, decision_block, prev_correct):
    ...
    if block_sequence_max_len_idx == 0:
        ...
        exploration_trials = np.concatenate(
            [np.ones(errors + 1), np.zeros(block_correct_decisions.size - errors - 1)]
        )
...
def get_reversal_block(block_correct_decisions, decision_block, prev_correct):
    ...
    if block_sequence_max_len_idx == 0:
        ...
        reversal_trials = np.concatenate(
            [np.ones(errors + 1), np.zeros(block_correct_decisions.size - errors - 1)]
        )
...
def get_consecutive_rewarded_block(
    block_correct_decisions, decision_block, prev_correct
):
    """
    ...
    consecutive_rewarded_trials = [
        np.zeros(block_sequence_list[i].size).astype(int)
        for i in range(len(block_sequence_list))
    ]
    consecutive_rewarded_trials[block_sequence_max_len_idx] = np.ones(
        block_sequence_list[block_sequence_max_len_idx].size
    ).astype(int)
    ...
    with open(save_file, "w") as f:
        ...
        for PHASE in [0, 1]:
            for subID in range(len(data1)):
                if (
                    mask_without_nan[subID] or 1
                ):  # initially if one time of this subject has a "nan" than do not use this subject... but I don't know why...
                    stringList = [
                        str(int(subID)),
                        str(data[PHASE, subID]),
                        str(int(PHASE)),
                    ]
                    for string in stringList:
                        print(string, end="\t", file=f)
