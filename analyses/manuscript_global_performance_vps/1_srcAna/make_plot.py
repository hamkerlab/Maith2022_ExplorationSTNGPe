...
from extra_functions import (
    get_output_vp,
    get_block_trials,
    get_block_trials_are_correct,
    get_initial_trials,
    plot_column,
    get_successful_block_list,
    get_how_long_until_rewarded,
    get_response_times,
    save_2_within_arrays,
    get_reversal_learning_idx,
    get_consecutive_rewarded_idx,
)
import sys
...
for vp_id in range(num_vps):
    ### GET DATA OF TRIALS
    trials, correct, decision, _, start, dopInp, block = get_output_vp(
        folder, vp_id + 1
    )
    ...
    pre_correct, new_correct, block_trials = get_block_trials(
        correct, decision, block, post_switch_trials, experiment
    )
    ### CHECK IF DECISIONS ARE PRE/NEW CORRECT ACTIONS
    block_is_pre_correct, block_is_new_correct = get_block_trials_are_correct(
        block_trials, pre_correct, new_correct
    )

    ### GET SUCCESSFUL BLOCKS
    successful_blocks = get_successful_block_list(correct, decision, block, experiment)
    ...
    pre_correct_number[vp_id] = np.mean(
        block_is_pre_correct[successful_blocks[1:].astype(bool), :], 0
    )
    new_correct_number[vp_id] = np.mean(
        block_is_new_correct[successful_blocks[1:].astype(bool), :], 0
    )

    ### FOR EACH VALID BLOCK HOW MANY ERRORS WERE MADE BEFORE NEW CORRECT WAS SELECTED CONSECUTIVELY
    how_long_until_rewarded = get_how_long_until_rewarded(
    ...
    if successful_blocks[0]:
        initial_trials[vp_id] = get_initial_trials(
            correct, decision, post_switch_trials
        )
    number_initial_errors[vp_id] = how_long_until_rewarded[0]

    ### THIRD: RESPONSE TIMES
    response_times = get_response_times(
        start, dopInp, decision, correct, block, experiment
    )
    reversal_idx = get_reversal_learning_idx(correct, decision, block, experiment)
    consecutive_rewarded_idx = get_consecutive_rewarded_idx(
        correct, decision, block, experiment
    )

    response_times_all.append(response_times)
    response_times_reversal.append(response_times[reversal_idx.astype(bool)])
    response_times_exploitation.append(
        response_times[consecutive_rewarded_idx.astype(bool)]
    )
...
response_times_all_per_vp = np.array(
    [np.median(rt_list[rt_list < 400]) for rt_list in response_times_all]
)  # per vp the median of RTs
time_outs_all_per_vp = [
    100 * np.mean(rt_list >= 400) for rt_list in response_times_all
]  # in percent
response_times_exploitation_per_vp = np.array(
    [np.median(rt_list[rt_list < 400]) for rt_list in response_times_exploitation]
)  # per vp the median of RTs
time_outs_exploitation_per_vp = [
    100 * np.mean(rt_list >= 400) for rt_list in response_times_exploitation
]  # in percent
response_times_reversal_per_vp = np.array(
    [np.median(rt_list[rt_list < 400]) for rt_list in response_times_reversal]
)  # per vp the median of RTs
time_outs_exploration_per_vp = [
    100 * np.mean(rt_list >= 400) for rt_list in response_times_reversal
]  # in percent
...
## INITIAL BLOCKS
plot_column(
    "Initial blocks", 0, initial_trials, post_switch_trials, bold_font, large_bold_font
)
## REVERSAL BLOCKS
plot_column(
    "Reversal blocks",
    1,
    [pre_correct_number, new_correct_number],
    post_switch_trials,
    bold_font,
    large_bold_font,
)
...
plt.savefig(f"../3_results/global_performance_{experiment}.svg")


### TEXT FILE
with open(f"../3_results/quantities_{experiment}.txt", "w") as f:
    ### SUCCESSFUL BLOCKS
    ...
    print(
        "successful_block_percent mean over vps:",
        np.mean(np.mean(successful_block_list, 1)),
        "\nSD over vps:",
        np.std(np.mean(successful_block_list, 1)),
        "\n",
        file=f,
    )
    ### AVERAGE ERRORS
    ...
    print(
        "initial:",
        np.nanmean(number_initial_errors, 0),
        np.nanstd(number_initial_errors, 0),
        file=f,
    )
    print(
        "reversal:",
        np.nanmean(number_errors_per_vp),
        np.nanstd(number_errors_per_vp),
        "\n",
        file=f,
    )
    ### RESPONSE TIMES ALL
    ...
    print(
        "timeouts: m =",
        round(np.mean(time_outs_all_per_vp), 2),
        "%, sd =",
        round(np.std(time_outs_all_per_vp), 2),
        "%\n",
        file=f,
    )
    ...
    print(
        "timeouts: m =",
        round(np.mean(time_outs_exploitation_per_vp), 2),
        "%, sd =",
        round(np.std(time_outs_exploitation_per_vp), 2),
        "%\n",
        file=f,
    )
    ...
    print(
        "timeouts: m =",
        round(np.mean(time_outs_exploration_per_vp), 2),
        "%, sd =",
        round(np.std(time_outs_exploration_per_vp), 2),
        "%\n",
        file=f,
    )
...
save_2_within_arrays(
    f"response_times_per_vp_{experiment}.txt",
    "RT",
    response_times_exploitation_per_vp,
    response_times_reversal_per_vp,
)
save_2_within_arrays(
    f"time_outs_per_vp_{experiment}.txt",
    "RT",
    time_outs_exploitation_per_vp,
    time_outs_exploration_per_vp,
)
save_2_within_arrays(
    f"number_of_errors_vps_{experiment}.txt",
    "ERRORS",
    number_initial_errors,
    number_errors_per_vp,
)
