...
def get_output_vp(x,vpID):
    ...
    _.sort()
    cluster = np.array([1,2,3,4,5])[np.histogram(correct,[1,2,3,4,5,10])[0]>_[1]]
...
def plot_column(title, col, selections, post_switch_trials, bold_font, large_bold_font):
    ...
    if col==0: plt.plot(trials, selections, color='k')
        ...
        plt.plot(trials, selections_pre, color='k', ls='dotted', label='previously rewarded')
...
def get_block_trials_are_cluster(block_trials, cluster):
    ...
    out_cluster1_block = np.array([out_cluster[0] for i in range(block_trials.shape[1])]).T
    out_cluster2_block = np.array([out_cluster[1] for i in range(block_trials.shape[1])]).T
    
    block_is_in_cluster  = np.equal(block_trials, in_cluster1_block).astype(int) + np.equal(block_trials, in_cluster2_block).astype(int) + np.equal(block_trials, in_cluster3_block).astype(int)

    block_is_out_cluster = np.equal(block_trials, out_cluster1_block).astype(int) + np.equal(block_trials, out_cluster2_block).astype(int)
...
def get_selected_cluster(correct, decision, block, cluster):
    """
        correct: numpy array with correct actions
        decision: numpy array with selected actions
        block: numpy array with block of experiment
    ...
    in_cluster_selections=np.zeros(block_start_end_list.shape[0])*np.nan
    out_cluster_selections=np.zeros(block_start_end_list.shape[0])*np.nan
    for idx, block_start_end in enumerate(block_start_end_list):
        start = block_start_end[0]
        end   = block_start_end[1]
        in_cluster_selections[idx] = np.sum(1*(cluster[0]==decision[start:end+1])+1*(cluster[1]==decision[start:end+1])+1*(cluster[2]==decision[start:end+1]))
        out_cluster_selections[idx] = np.sum(1*(out_cluster[0]==decision[start:end+1])+1*(out_cluster[1]==decision[start:end+1]))
...
def get_fake_selections(block_n=45, repeat_prob = 0.01, nothing_prob = 0.02, bias = 0, rng=np.random.default_rng()):
    """
        get fake selections
...
def get_p(available, bias, resp):
    if resp in available:
        if resp == 0:
            ...
            prob=(1-get_p(available, bias, 0))/(len(available)-int(0 in available)) * (1 - bias)
        elif resp==1 or resp==2:
            prob=(1 - get_p(available, bias, 0) - get_p(available, bias, 3) - get_p(available, bias, 4)) / ( 2*(int(1 in available)*int(2 in available))+1*int(not(bool(int(1 in available)*int(2 in available)))) )
...
def get_centered_selected_out_cluster_m(arr):
    ...
    return [selected_out_cluster_m, selected_out_cluster_sd, last_positive_idx, selected_out_cluster]

def make_regression_plot(title, selected_out_cluster_m, selected_out_cluster_sd, plot=1, bold_font=None):
    ### linear regression
    ...
    if plot:
        ...
        plt.errorbar(x_plot, selected_out_cluster_m, yerr=selected_out_cluster_sd, fmt='k.')
...
def make_single_vps_plot(title,arr, bold_font):
    plt.figure(figsize=(8.5/2.54,7/2.54))
    x=np.arange(arr.shape[1])
    x_rev=np.arange(-arr.shape[1],0)
    for idx in range(arr.shape[0]):
        plt.subplot(arr.shape[0],1,idx+1)
        if np.sum(np.isnan(arr[idx]))>0:
            plt.bar(x_rev,arr[idx])
            plt.axvspan(x_rev[0]-1, x_rev[0]+np.where(np.isnan(arr[idx]))[0][-1], color='grey', alpha=0.3)
    ...
    plt.xlabel('blocks', **bold_font)
    plt.text(0, 0.5, 'number of responses', ha='center', va='center', rotation='vertical', transform=plt.gcf().transFigure, **bold_font)

    plt.tight_layout()
    plt.savefig(title)


def get_fake_data(num_vps, num_valid_rs, learned_rule_range, learned_rule_arr, random_rule_learn_time=0, rng=np.random.default_rng()):
    ### create fake data
    ...
    for idx in range(num_vps):
        ...
        if random_rule_learn_time:
            rule_learned = rng.integers(low=learned_rule_range[0], high=learned_rule_range[1], endpoint=True)
        else:
            rule_learned = learned_rule_arr[idx]

        ### create the dataset, two phases 1:wihtout bias, 2: with bias = sudden rule learning
        selections_phase_a = get_fake_selections(block_n=rule_learned, repeat_prob = 0.1, nothing_prob = 0.02, bias = 0, rng=rng)
        selections_phase_b = get_fake_selections(block_n=num_valid_rs+1-rule_learned, repeat_prob = 0.1, nothing_prob = 0.02, bias = 1, rng=rng)
        selections_phase_a, selections_phase_b = make_same_len(selections_phase_a, selections_phase_b)
        block_trials = np.concatenate([selections_phase_a,
                                       selections_phase_b])

        ###get how often out cluster was selected for each block = block_is_out_cluster    
        cluster=[1,2,5]# in fake selections the third in cluster response, i.e. the previously rewarded, does not exist, there are only [0,1,2,3,4] and 1,2 are cluster --> just use 5 as third
...
def make_same_len(arr_a, arr_b):
    ...
    while arr_a.shape[1]<arr_b.shape[1]:
        arr_a = np.insert(arr_a, np.shape(arr_a)[1], 1, 1)# insert column of ones at the end
    while arr_b.shape[1]<arr_a.shape[1]:
        arr_b = np.insert(arr_b, np.shape(arr_b)[1], 1, 1)# insert column of ones at the end
        
    return [arr_a, arr_b]

def make_correlation_plot(title, arr, plot=1, bold_font=None):
    """
    ...
    CI_fisher_transformed = np.arctanh(r) + np.array([-1,1])* z_crit * (1/np.sqrt(n-3))
    ...
    if plot:
        ...
        if p_val>=0.001:
            plt.text(0.97,0.97,'$r$ = '+str(round(r,2))+', $p$ = .'+str(round(p_val,3)).split('.')[1],ha='right',va='top', transform=plt.gca().transAxes)
        else:
            plt.text(0.97,0.97,'$r$ = '+str(round(r,2))+', $p$ < .001',ha='right',va='top', transform=plt.gca().transAxes)
        plt.tight_layout()
        plt.savefig(title)
