...
setup(dt=0.1, num_threads=1)


sim_id = int(sys.argv[1])
morph = 2 * ((sim_id - 1) // 400)  # int(sys.argv[2])
print(f"sim_id {sim_id}, morph {morph/100.}%")
...
stn_gpe_maxw = 0.00145
stn_gpe_pattern = "morphing"

### stn_gpe_mw_list indizes: [pre_pop, post_pop]
...
if stn_gpe_pattern == "morphing":
    stn_gpe_mw_high = stn_gpe_mw * (1 + morph / 100.0)
    stn_gpe_mw_low = stn_gpe_mw * (1 - morph / 100.0)
    stn_gpe_mw_list = np.array(
        [
            [stn_gpe_mw, stn_gpe_mw_high, stn_gpe_mw_high, stn_gpe_mw_high, stn_gpe_mw],
            [stn_gpe_mw_low, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw_low],
            [stn_gpe_mw_low, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw_low],
            [stn_gpe_mw_low, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw, stn_gpe_mw_low],
            [stn_gpe_mw, stn_gpe_mw_high, stn_gpe_mw_high, stn_gpe_mw_high, stn_gpe_mw],
        ]
...
STN_learning.max_weight = stn_gpe_maxw
...
def csd1_min_weight():
    return CSD1.min_weight[0]


def csd1_max_weight():
    return CSD1.max_weight[0]


def csd2_min_weight():
    return CSD2.min_weight[0]


def cstn_max_weight():
    return CSTN.max_weight[0]


def cstn_rand_weight():
    return np.random.uniform(low=0.00034, high=0.00037)


def ct_rand_weight(mode):
    if mode == "l":
        return np.random.uniform(low=7.5e-8, high=1.06e-7)
    if mode == "h":
        return np.random.uniform(low=5e-5, high=6e-5)
...
for block in range(num_blocks):
    num_trials = 0
    correct = 99
    print(f"prev_correct = {prev_correct+1}")
    print("new_correct = ", correct + 1, "\n")

    ### reward all except prev_correct
    ### --> first explored action is rewarded
    probabilities_reward = np.ones(5)
    probabilities_reward[prev_correct] = 0
    num_correct[0] = 0
    num_zeros = 0
    # this will run trials until 1 correct responses are given or for a maximum of 30 trials
    while num_correct[0] < 1 and num_trials < 30 and num_zeros < 3:
        ...
        if decision > 0:
            ...
            num_zeros += 1
            num_correct[stim_id] = 0
            dopamine_level = -dopamine_rate / 2.0
...
np.savetxt(
    ...
    delimiter="\t",
    newline="\n",
    fmt=["%1d", "%1.1f", "%1.1f", "%1.1f", "%1d", "%1d", "%1e", "%1.2f", "%1d"],
