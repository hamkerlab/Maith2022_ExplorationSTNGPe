...
stn_gpe_factor, stn_gpe_post_factor = [
    [0.13, 10],
    [0.14, 10],
    [0.15, 10],
    [0.16, 10],
...
STN_learning = Synapse(
    ...
    factor            = 0.12     : postsynaptic
    post_factor       = 10      : postsynaptic
...
# Integrators
Integrators = Population(
    num_actions, Integrator_neuron, stop_condition=""" decision>0 """
)
...
# Integrator set up
Integrators.neuron_id = np.arange(1, num_actions + 1)
        ...
        for pre_rank in range(pre.size):
            ...
            if pre_population == 0 or pre_population == 4:
                ...
                if pre_population == post_population:
                    prob = probability[0]
                if (
                    post_population != 0 and post_population != 4
...
CSD1 = Projection(
    pre=Cortex, post=SD1, target="ampa", synapse=IzhikevichSynapse
).connect_all_to_all(weights=Normal(0.0004, 0.00004), delays=0)

CSD2 = Projection(
    pre=Cortex, post=SD2, target="ampa", synapse=IzhikevichSynapse_D2
).connect_all_to_all(weights=0.0, delays=0)

CSTN = Projection(
    pre=Cortex, post=STN, target="ampa", synapse=IzhikevichSynapse_STN
).connect_all_to_all(weights=0, delays=0)

CT = Projection(
    pre=Cortex, post=Thalamus, target="ampa", synapse=STDP_online
).connect_all_to_all(weights=0, delays=0)

# GPi STN GPe

D1GPi = Projection(
    pre=SD1, post=GPi, target="gaba", synapse=FixedSynapse
).connect_with_func(
...
D2GPe = Projection(
    pre=SD2, post=GPe, target="gaba", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.005,
    sd_weight=0.0,
...
GPeSTN = Projection(
    pre=GPe, post=STN, target="gaba", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.007,
    sd_weight=0,
...
if stn_gpe_synapse == "fixed":
    STNGPe = Projection(
        pre=STN, post=GPe, target="ampa", synapse=FixedSynapse
    ).connect_with_func(
        method=probabilistic_pattern_STNGPe,
        mean_weight_list=stn_gpe_mw_list,
        sd_weight=0.0,
        probability=[0.05, 0.3],
    )
if stn_gpe_synapse == "plastic":
    STNGPe = Projection(
        pre=STN, post=GPe, target="ampa", synapse=STN_learning
    ).connect_with_func(
        method=probabilistic_pattern_STNGPe,
        mean_weight_list=stn_gpe_mw_list,
        sd_weight=0.0,
...
STNGPi = Projection(
    pre=STN, post=GPi, target="ampa", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.0008,
    sd_weight=0,
...
GPeGPi = Projection(
    pre=GPe, post=GPi, target="gaba", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.0055,
    sd_weight=0,
...
GPiThal = Projection(
    pre=GPi, post=Thalamus, target="gaba", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.0007,
    sd_weight=0,
...
# feedback synapses
ThalSD2 = Projection(
    pre=Thalamus, post=SD2, target="ampa", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern, mean_weight=0.001, sd_weight=0, probability=[0.4, 0.0]
)  # 0.0015

ThalSD1 = Projection(
    pre=Thalamus, post=SD1, target="ampa", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern, mean_weight=0.001, sd_weight=0, probability=[0.4, 0.0]
)  # 0.0015
...
STNESTN = Projection(
    pre=STNE, post=STN, target="ampa", synapse=FixedSynapse
).connect_one_to_one(weights=0.19)

GPiEGPi = Projection(
    pre=GPiE, post=GPi, target="ampa", synapse=FixedSynapse
).connect_one_to_one(weights=0.11)

GPeEGPe = Projection(
    pre=GPeE, post=GPe, target="ampa", synapse=FixedSynapse
).connect_one_to_one(weights=0.015)

SDNSD1 = Projection(
    pre=SDN, post=SD1, target="ampa", synapse=FixedSynapse
).connect_one_to_one(weights=0.015)

SDNSD1 = Projection(
    pre=SDN, post=SD1, target="gaba", synapse=FixedSynapse
).connect_one_to_one(weights=0.015)

SDNSD2 = Projection(
    pre=SDN, post=SD2, target="ampa", synapse=FixedSynapse
).connect_one_to_one(weights=0.015)

SDNSD2 = Projection(
    pre=SDN, post=SD2, target="gaba", synapse=FixedSynapse
).connect_one_to_one(weights=0.015)

ThalNThal = Projection(
    pre=ThalN, post=Thalamus, target="ampa", synapse=FixedSynapse
...
SD1SD1 = Projection(
    pre=SD1, post=SD1, target="gaba", synapse=FixedSynapse
).connect_with_func(
    method=probabilistic_pattern,
    mean_weight=0.015,
    sd_weight=0,
    probability=[0.0, 0.35],
)  # 0.035
...
ThalInt = Projection(
    pre=Thalamus, post=Integrators, target="ampa", synapse=FixedSynapse
...
class dopamine_cl:
    def __init__(self) -> None:
        self.reward_prediction = np.zeros(num_actions)
        self.tau_selected = t_dop

    def positive_dopamine(self, action_idx):
        """
        function called after a rewarded selection
        to obtain the dopamine signal (dopamine increase)
        and update the dopamine rates of the actions (reward prediciton error)

        action_idx: idx of selected (rewarded) action

        ret: the dopamine rate of the selected action
        """
        reward = dopamine_rate
        return self.get_dopamine(action_idx, reward)

    def negative_dopamine(self, action_idx=None):
        """
        function called after a not-rewarded selection (or no selection)
        to obtain the dopamine signal (dopamine decrease)
        and update the dopamine rates of the actions (reward prediciton error)

        action_idx: idx of selected (not-rewarded) action, if None --> no selection

        ret: the dopamine rate of the selected action
        """
        if action_idx == None:
            return -dopamine_rate / 2
        else:
            reward = -dopamine_rate
            return self.get_dopamine(action_idx, reward)

    def get_dopamine(self, action_idx, reward):
        """
        function called after a rewarded selection
        to obtain the dopamine signal (dopamine increase)
        and update the dopamine rates of the actions (reward prediciton error)
        ...
        ret: the dopamine rate of the selected action
        """
        ### calculate reward_prediction error of current selection
        reward_pred_error = reward - self.reward_prediction[action_idx]

        ### updated reward prediciton towards reward
        self.update_reward_prediction(action_idx, reward, self.tau_selected)

        ### return reward prediction error of selected action
        ### clip absolute of reward prediction (should not get to small)
        ### for negative rpe just use -dopamine_rate... the model needs it to unlearn correctly
        if reward > 0:
            return np.clip(reward_pred_error, dopamine_rate * 0.2, None)
        else:
            return -dopamine_rate

    def update_reward_prediction(self, action_idx, reward, tau):
        """
        change toward reward
        """
        self.reward_prediction[action_idx] = np.clip(
            (self.reward_prediction[action_idx] + tau * reward),
            -dopamine_rate,
            dopamine_rate,
        )
...
def mean_weights(synapses):
    return rebin(
        np.clip(np.array(synapses.w), synapses.min_weight[0], synapses.max_weight[0]),
        [num_actions, num_stimulus],
    )
...
def mean_weights_sg(synapses, prob):
    ...
    m = np.clip(
        synapses.connectivity_matrix(), synapses.min_weight[0], synapses.max_weight[0]
    )
    ...
    for i in range(num_actions):
        for j in range(num_actions):
            means[i, j] = np.mean(
                m[
                    i * population_size : (i + 1) * population_size,
                    j * population_size : (j + 1) * population_size,
                ]
            )
    return means
...
def get_rare_correct_list():
    ### generate a standard correct list with 2,3,4 rewarded and 1,5 never rewarded
    ### length = 60
    correct = np.random.choice(
        [0, 1, 2, 3, 4], size=1, replace=True, p=np.array([0, 1 / 3, 1 / 3, 1 / 3, 0])
    )[0]
    correct_list = []
    ...
    ### now all 12 blocks add a 1 and a 5
    ### --> resulting correct list = 70 blocks, 14*5
    splitted_correct_list = np.split(correct_list, 5)
    for i in range(5):
        min_idx = 0
        max_idx = 12
        idx = np.random.randint(low=min_idx, high=max_idx)
        splitted_correct_list[i] = np.insert(splitted_correct_list[i], idx, 0)
        idx = np.random.randint(low=min_idx, high=max_idx + 1)
        splitted_correct_list[i] = np.insert(splitted_correct_list[i], idx, 4)
    correct_list = np.concatenate(splitted_correct_list)
...
def make_correct_list(num_blocks):
    ...
    #    [4]*5,
    # ]
    correct_list = get_rare_correct_list()
    hist = np.histogram(correct_list, bins=[0, 1, 2, 3, 4, 5])[0]
    while (
        not ((hist == np.array([5, 20, 20, 20, 5])).all() == True)
        or sum(np.diff(correct_list) == 0) > 0
    ):
        correct_list = get_rare_correct_list()
        hist = np.histogram(correct_list, bins=[0, 1, 2, 3, 4, 5])[0]

    # correct_list = np.concatenate(correct_list)
    # while (hist==np.array([5,20,20,20,5])).any()==True:
    #    np.random.shuffle(correct_list)
...
def weights(means, stds):
    ...
    for corNeuron in range(population_size):
        ...
        for pop in range(num_actions):
            ...
            weights[
                pop * population_size : (pop + 1) * population_size, corNeuron
            ] = np.random.normal(means[pop], stds[pop], population_size)
...
for block in range(num_blocks):
    ...
    while num_correct[0] < 15 and num_trials < 30 and num_zeros < 3:
        ...
        Cortex[
            stim_id * population_size : (stim_id + 1) * population_size
        ].rates = input_rate
...
np.savetxt(
    "output_sim" + str(sim_id),
    output,
    delimiter="\t",
    newline="\n",
    fmt=["%1d", "%1.1f", "%1.1f", "%1.1f", "%1d", "%1d", "%1e", "%1.2f"],
    header="Trial\tStart\tDop_Input\tEnd\tcorrect\tdecision\tdopamine\ttime\t\tstn_gpe:"
    + stn_gpe_pattern,
)
